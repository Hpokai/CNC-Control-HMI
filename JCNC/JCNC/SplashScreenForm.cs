...
namespace JCNC
{
    public partial class SplashScreenForm : Form
    {
        ...
        /// <param name="Text">Message</param>
                public void UdpateCopyRight(string Text)
        {
            if (InvokeRequired)
            {
                // We're not in the UI thread, so we need to call BeginInvoke
                BeginInvoke(new StringParameterDelegate(UdpateCopyRight), new object[] { Text });
                return;
            }
...
//            label2.ForeColor = Color.Black;
  //          label2.Text = Text;
        ...
        /// </summary>
        /// <param name="Text">Message</param>
        public void UdpateVersionText(string Text)
        {
            if (InvokeRequired)
            {
                // We're not in the UI thread, so we need to call BeginInvoke
                BeginInvoke(new StringParameterDelegate(UdpateVersionText), new object[] { Text });
                return;
            }
            // Must be on the UI thread if we've got this far
